"""
Full production run — Uber Eats scraper.
...
Outputs:
  • logs/ubereats_full_run.log   — full DEBUG log
  • data/processed/ubereats_data.csv — one row per (location × restaurant)
  • Console summary table at the end

Usage:
    python run_ubereats_full.py [--batch-size N] [--dry-run]
...
LOG_FILE = LOGS_DIR / "ubereats_full_run.log"
CSV_FILE = PROCESSED_DIR / "ubereats_data.csv"
...
def _setup_logging() -> logging.Logger:
    ...
    return logging.getLogger("ubereats_full")
...
from scrapers.base import ScrapeResult, RestaurantResult, ProductResult
...
from config.settings import PROCESSED_DIR as _CFG_PROCESSED  # sanity check only
...
BLOCK_WAIT_ENV        = "600"  # production block-wait: 10 min (overridable)
...
def _print_console_table(all_rows: list[dict]) -> None:
    ...
    print(f"\n{'═'*len(hdr)}")
    print("UBER EATS — FULL RUN RESULTS")
    print(f"{'═'*len(hdr)}")
    print(hdr)
    print(divider)
...
async def scrape_with_retry(
    ...
    for attempt in (1, 2):
        ...
        if attempt == 1:
            ...
            # Reset the scraper's block counter so the retry gets a clean slate
...
async def run_full(batch_size: int) -> list[dict]:
    ...
    """
    os.environ.setdefault("UBEREATS_BLOCK_WAIT_SECS", BLOCK_WAIT_ENV)
    ...
    logger.info("=" * 70)
    logger.info("Uber Eats — Full production run")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Uber Eats full production scrape")
    p.add_argument(
