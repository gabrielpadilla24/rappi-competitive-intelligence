"""
Full production run — DiDi Food scraper.
...
Execution model:
  • Locations are processed in batches of BATCH_SIZE (default 3).
  ...
  • A single browser session is reused within a batch; reset between batches.
...
Outputs:
  • logs/didifood_full_run.log        — full DEBUG log
  • data/processed/didifood_data.csv  — one row per (location × restaurant)
  • Console summary table at the end

Usage:
    python run_didifood_full.py [--batch-size N] [--dry-run]

    --batch-size N  Override batch size (default 3)
    --dry-run       Print execution plan and exit without scraping
"""
...
LOG_FILE = LOGS_DIR / "didifood_full_run.log"
...
def _setup_logging() -> logging.Logger:
    ...
    return logging.getLogger("didifood_full")
...
BATCH_PAUSE_LO        = 45.0
BATCH_PAUSE_HI        = 90.0
RETRY_WAIT_SECS       = 60     # 1 minute on failure
...
def _print_console_table(all_rows: list[dict]) -> None:
    ...
    print(f"\n{'═'*len(hdr)}")
    print("DiDi FOOD — FULL RUN RESULTS")
    print(f"{'═'*len(hdr)}")
    print(hdr)
    print(divider)
...
async def run_full(batch_size: int) -> list[dict]:
    ...
    logger.info("=" * 70)
    logger.info("DiDi Food — Full production run")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="DiDi Food full production scrape")
    p.add_argument(
